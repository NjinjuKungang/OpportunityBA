    ...
    <div class="mx-auto max-w-3xl pt-8 ">
      <div class="flex justify-center py-12 bg-gray-100 shadow-lg rounded shadow-amber-200">
          <div class="grid gap-5 justify-items-left">
              ...
              <input class="h-10 px-2 rounded-md" type="text" placeholder="Enter full names" name="name" value="{{old('name')}}">
              @error('name')
                    <span class="text-red-500 italic">*{{$message}}</span>
                @enderror
                <label class="text-xl">Contact:</label>
                <input class="h-10 px-2 rounded-md" type="text" placeholder="Phone number" name="phone" value="{{old('phone')}}">
                @error('phone')
                      <span class="text-red-500 italic">*{{$message}}</span>
                @enderror
                <label class="text-xl">Email:</label>
                <input class="h-10 px-2 rounded-md" type="email" placeholder="Email address" name="email" value="{{old('email')}}">
                @error('email')
                    <span class="text-red-500 italic">*{{$message}}</span>
                @enderror
