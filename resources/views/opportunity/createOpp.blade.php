...
<body>
  ...
  <form action="{{ route('opportunity')}}" method="post" enctype="multipart/form-data">
    @csrf
    <div class="mx-auto max-w-xl">
      <div class="flex justify-center py-20 bg-gray-100 shadow-lg shadow-amber-200">
          <div class="grid gap-5 justify-items-left">
              ...
              <input class="h-10 px-2 italic rounded-md" type="text" placeholder="Enter title" name="title" id="">
              @error('title')
              ...
              <textarea class="rounded-md italic p-1" rows="5" cols="50" placeholder="Description about opportunity" name="title" name="description"></textarea>
              @error('description')
                    <span class="text-red-500 italic">*{{$message}}</span>
                @enderror
              ...
              <button type="submit" class="grid justify-items-center bg-amber-600 rounded ring-2 ring-amber-500 hover:ring-gray-600/50 p-2 font-semibold">Create</button>
          </div>
        </div>
    </div>
  </form>
